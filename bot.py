...
**1 — GET IDENTIFIED** — Run **`/link-game`** and connect your HLL: Vietnam game identity.
**2 — REPORT IN** — Run **`/apply`** and complete the short Battalion Clerk intake right here in Discord.
**3 — STAND BY FOR ORDERS** — Once Command accepts your case, you'll move to **Ready to Assign** until your Company, Platoon, and Squad are filed.

If an active 1/5 Cav member recruited you, the intake will let you select that Soldier so the referral is attached to your Recruiting Case.

No mandatory operation nights. No drawn-out processing. Get identified, get processed, and get in the fight.
...
async def _ensure_battalionbrief_config_table():
    await collector.start(); db=collector.db
    await db.execute("""CREATE TABLE IF NOT EXISTS clerk_battalionbrief_config(
        guild_id TEXT PRIMARY KEY,channel_id TEXT,enabled BOOLEAN NOT NULL DEFAULT TRUE,
        weekday INTEGER NOT NULL DEFAULT 6,hour INTEGER NOT NULL DEFAULT 19,minute INTEGER NOT NULL DEFAULT 0,
        mention_role_id TEXT,include_fund BOOLEAN NOT NULL DEFAULT TRUE,updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW())""")

async def _brief_config(guild_id:int):
    await _ensure_battalionbrief_config_table(); db=collector.db
    row=await db.fetchrow('SELECT * FROM clerk_battalionbrief_config WHERE guild_id=$1',str(guild_id))
    return dict(row) if row else None

async def _save_brief_config(guild_id:int,*,channel_id=None,enabled=None,weekday=None,hour=None,minute=None,mention_role_id=None,include_fund=None):
    await _ensure_battalionbrief_config_table(); db=collector.db
    current=await _brief_config(guild_id) or {}
    values={'channel_id':str(channel_id or current.get('channel_id') or ''),'enabled':bool(current.get('enabled',True) if enabled is None else enabled),
            'weekday':int(current.get('weekday',6) if weekday is None else weekday),'hour':int(current.get('hour',19) if hour is None else hour),
            'minute':int(current.get('minute',0) if minute is None else minute),'mention_role_id':str(mention_role_id or current.get('mention_role_id') or '') or None,
            'include_fund':bool(current.get('include_fund',True) if include_fund is None else include_fund)}
    await db.execute("""INSERT INTO clerk_battalionbrief_config(guild_id,channel_id,enabled,weekday,hour,minute,mention_role_id,include_fund,updated_at)
        VALUES($1,$2,$3,$4,$5,$6,$7,$8,NOW()) ON CONFLICT(guild_id) DO UPDATE SET channel_id=EXCLUDED.channel_id,
        enabled=EXCLUDED.enabled,weekday=EXCLUDED.weekday,hour=EXCLUDED.hour,minute=EXCLUDED.minute,
        mention_role_id=EXCLUDED.mention_role_id,include_fund=EXCLUDED.include_fund,updated_at=NOW()""",
        str(guild_id),values['channel_id'],values['enabled'],values['weekday'],values['hour'],values['minute'],values['mention_role_id'],values['include_fund'])
    return values

BRIEF_DAY_CHOICES=[app_commands.Choice(name=n,value=str(i)) for i,n in enumerate(['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'])]

async def _build_battalion_brief(guild:discord.Guild,include_fund=True):
    data=await web.request('GET','/internal/clerk/reports/weekly-battalion')
    w=data.get('weekly') or {}; notable=data.get('notable') or []
    try: tz=ZoneInfo(BATTALION_TIMEZONE)
    except Exception: tz=ZoneInfo('America/New_York')
    now=datetime.now(tz); week_end=now.strftime('%d %b %Y').upper()
    lines=[f"**1/5 CAV — WEEKLY BATTALION BRIEF**",f"**WEEK ENDING: {week_end}**",'',
    ...
    if upcoming:
        for e in upcoming[:2]: lines.append(f"• Upcoming: **{e.get('title') or 'Battalion event'}**")
    else: lines.append('• No upcoming event currently filed.')
    fund=w.get('fund') or {}
    if include_fund and fund:
        lines += ['', '**BATTALION FUND**', f"• Current balance: **${float(fund.get('balance',0)):,.2f}**", f"• Monthly goal: **${float(fund.get('monthly_goal',0)):,.2f}**", f"• Contributions this month: **${float(fund.get('month_contributions',0)):,.2f}**"]
    if notable:
        lines += ['', '**FROM THE FIELD**']
        for n in notable[:4]: lines.append(f"• **{n.get('name','Soldier')}** — {n.get('detail','milestone reached')}")
...
async def battalionbrief_setup(interaction:discord.Interaction,channel:discord.TextChannel,day:app_commands.Choice[str],hour:app_commands.Range[int,0,23]=19,mention_role:Optional[discord.Role]=None,include_fund:bool=True):
    if not await require_manage_guild(interaction): return
    cfg=await _save_brief_config(interaction.guild_id,channel_id=channel.id,enabled=True,weekday=int(day.value),hour=int(hour),minute=0,mention_role_id=mention_role.id if mention_role else None,include_fund=include_fund)
...
async def battalionbrief_preview(interaction:discord.Interaction):
    if not await require_manage_guild(interaction): return
    await interaction.response.defer(ephemeral=True)
    cfg=await _brief_config(interaction.guild_id) or {}
...
async def nco_accountability_channel(interaction:discord.Interaction, channel:discord.TextChannel):
    ...
    await interaction.response.send_message(f'NCO / Company accountability rollups will be posted to {channel.mention}.',ephemeral=True)
...
async def weekly_battalion_report_watch():
    ...
    for guild in bot.guilds:
        ...
        try:
            ...
            if now.weekday()!=int(cfg.get('weekday') or 0) or now.hour!=int(cfg.get('hour') or 0) or now.minute< int(cfg.get('minute') or 0) or now.minute>=int(cfg.get('minute') or 0)+5: continue
            report_key=f"{now.isocalendar().year}-W{now.isocalendar().week:02d}-{now.weekday()}-{now.hour:02d}{int(cfg.get('minute') or 0):02d}"
            if not await _notice_once(guild.id,'BATTALION','WEEKLY_BATTALION_BRIEF',report_key): continue
            ch=guild.get_channel(int(cfg.get('channel_id') or 0))
                    ...
                    if state=='OK' and old in {'WARN','FAIL'}:
                        ...
                        for c in data.get('checks') or []:
                            if str(c.get('status') or '').upper() in {'WARN','FAIL'}:
                                problems.append(f"• **{c.get('name')}** — {c.get('status')}: {c.get('detail')}\n  ↳ **FIX:** {c.get('recommendation') or 'Review the Command System Health page.'}")
...
@bot.tree.command(name='first30-channel', description='Assign the NCO channel for non-mandatory first-30-day member retention alerts.')
async def first30_channel(interaction:discord.Interaction, channel:discord.TextChannel):
    if not await require_manage_guild(interaction): return
    await set_report_channel(interaction.guild_id,'FIRST30_RETENTION',channel.id)
    await interaction.response.send_message(f'First-30-day NCO retention alerts will be posted to {channel.mention}. These are check-ins, not mandatory participation orders.',ephemeral=True)

@bot.tree.command(name='first30-status', description='Preview new members in the first-30-day retention lane.')
async def first30_status(interaction:discord.Interaction):
    if not await require_manage_guild(interaction): return
    await interaction.response.defer(ephemeral=True)
    data=await web.request('GET','/internal/clerk/retention/first30')
    rows=data.get('members') or []
    lines=[]
    for r in rows[:20]:
        issues=', '.join(r.get('issues') or []) or 'ON TRACK'
        lines.append(f"• **{r.get('name')}** — Day {r.get('age_days',0)} — {r.get('unit') or 'UNASSIGNED'} — {issues}")
    await interaction.followup.send((f"**1/5 CAV — FIRST 30 DAYS**\nMembers: **{len(rows)}** | Needs NCO attention: **{data.get('attention_count',0)}**\n\n"+('\n'.join(lines) if lines else 'No members currently in the first-30-day lane.'))[:1950],ephemeral=True)

@tasks.loop(minutes=30)
async def first30_retention_watch():
    if not WEBSITE_BASE_URL or not CLERK_SYNC_KEY: return
    for guild in bot.guilds:
        if GUILD_ID and guild.id != GUILD_ID: continue
        try:
            ch=await get_report_channel(guild,'FIRST30_RETENTION')
            if not ch: continue
            data=await web.request('GET','/internal/clerk/retention/first30')
            for r in (data.get('attention') or []):
                issues=r.get('issues') or []
                # One notice per Soldier/issue signature; changed problems can surface again.
                sig='|'.join(sorted(issues)) or 'ON_TRACK'
                key=f"FIRST30:{r.get('personnel_id')}:{sig}"
                if not await _notice_once(guild.id,str(r.get('personnel_id')),'FIRST30_RETENTION',key): continue
                opp=r.get('opportunities') or []
                opp_line=''
                if opp:
                    opp_line='\n**Closest progression:** '+', '.join(f"{x.get('name')} ({x.get('progress',0)}%)" for x in opp[:3])
                body=(f"**1/5 CAV — NEW MEMBER CHECK-IN**\n**{r.get('name')}** — Day **{r.get('age_days',0)}** — {r.get('unit') or 'UNASSIGNED'}\n"
                      f"**Needs attention:** {', '.join(issues)}\n**NCO next step:** {r.get('next_step')}"
                      f"{opp_line}\n\n*This is a support/check-in notice. The 1/5 Cav does not require a mandatory play schedule.*")
                await ch.send(body[:1900])
        except Exception as exc: log.warning('[FIRST30 RETENTION WATCH FAILED] guild=%s error=%s',guild.id,exc)

@first30_retention_watch.before_loop
...
@bot.tree.command(name='server-watch-status', description='Show Battalion Clerk suspicious-player monitoring status for both official HLL servers.')
...
@bot.tree.command(name='server-watch-test', description='Test Server Watch DMs and the shared Server Watch text channel.')
...
def _normalize_hll_side(team_id, match_row):
    raw=str(team_id or '').strip()
    if not raw:
        return None
    key=re.sub(r'[^a-z0-9]+','',raw.lower())
    allied_faction=re.sub(r'[^a-z0-9]+','',str((match_row or {}).get('allied_faction_id') or '').lower())
    axis_faction=re.sub(r'[^a-z0-9]+','',str((match_row or {}).get('axis_faction_id') or '').lower())
    if key in {'allied','allies','us','usa','unitedstates','unitedstatesarmy','american'}:
        return 'US'
    if key in {'axis','nva','northvietnam','northvietnamese','peoplesarmyofvietnam'}:
        return 'NVA'
    if allied_faction and key == allied_faction:
        return 'US'
    if axis_faction and key == axis_faction:
        return 'NVA'
    if allied_faction == '2' and axis_faction == '1':
        if key == '2': return 'US'
        if key == '1': return 'NVA'
    return None
...
async def _live_ready_room_side_muster(match_id, members):
    ...
    *whether that opted-in member is actually in the live round and which side
    ...
    rows=await collector.db.fetch("""
        ...
        JOIN hll_personnel_links l ON l.personnel_id=s.personnel_id AND l.verified=TRUE
        WHERE s.match_id=$1 AND l.discord_user_id IS NOT NULL
...
async def _publish_match_formation(guild,cfg,match_id=None,automatic=True):
    ...
    ready_members=_formation_members(ready)
    if len(ready_members) < MATCH_FORMATION_MIN_PLAYERS:
        return {'ok':False,'error':f'Only {len(ready_members)} player(s) are in {ready.name}; 6 are required.','player_count':len(ready_members)}
    ...
    if auto_side:
        ...
        muster=await _live_ready_room_side_muster(match_id,ready_members)
        side=_select_unit_combat_side(muster)
        if not side:
            return {'ok':False,'error':f"Unable to determine one 1/5 Cav combat side from live HLL telemetry (U.S. {len(muster['US'])} / NVA {len(muster['NVA'])}). No one was moved."}
        ...
        side=str(cfg.get('side_mode') or 'US').upper(); members=ready_members
    ...
    await collector.db.execute("""INSERT INTO clerk_match_formation_history
        ...
        guild.id,match_id,ready.id,text.id,side,len(members),json.dumps(packed))
    ...
    return {'ok':True,'message':msg,'player_count':len(members),'side':side,'roster':roster,'routing':routing,'skipped':skipped,'unknown':unknown}
...
async def match_formation_watch():
    ...
    try:
        ...
        latest_done=await _latest_completed_hll_match_id(); latest_active=await _latest_active_hll_match_id()
        for row_obj in rows:
...
@app_commands.choices(side=[app_commands.Choice(name='Automatic — detect from HLL server',value='AUTO'),app_commands.Choice(name='U.S. side — manual fallback',value='US'),app_commands.Choice(name='NVA side — manual fallback',value='NVA')])
async def combat_side(interaction:discord.Interaction,side:app_commands.Choice[str]):
    if not interaction.user.guild_permissions.manage_guild:
        await interaction.response.send_message('Manage Server permission is required.',ephemeral=True); return
    await ensure_match_formation_schema()
    auto=side.value=='AUTO'; stored='US' if auto else side.value
    await collector.db.execute("UPDATE clerk_match_formation_config SET side_mode=$2,auto_side=$3,updated_by=$4,updated_at=NOW() WHERE guild_id=$1",interaction.guild_id,stored,auto,interaction.user.id)
    if auto:
        msg='Combat side detection set to **AUTOMATIC**. Battalion Clerk will use linked game identities and live HLL telemetry at the next match start.'
    else:
        extra='Helicopter 1–3 are eligible.' if side.value=='US' else 'Helicopter 1–3 are HARD DISABLED.'
        msg=f'Combat side manual fallback set to **{side.name}**. {extra}'
    await interaction.response.send_message(msg,ephemeral=True)
...
async def combat_generate(interaction:discord.Interaction):
    ...
    await interaction.followup.send(f"Combat roster generated: **{result['player_count']}** mustered. Voice routing begins after the **60-second staging delay**; **{result['routing']['moved']}** moved.",ephemeral=True)
...
class RecruiterMemberSelect(discord.ui.UserSelect):
    def __init__(self):
        super().__init__(placeholder='Select the 1/5 Cav member who recruited you',min_values=1,max_values=1,custom_id='recruit_referrer_member_select')
...
class RecruiterMemberSelectView(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=None)
        self.add_item(RecruiterMemberSelect())


class RecruitReferralView(discord.ui.View):
    def __init__(self): super().__init__(timeout=None)

    @discord.ui.button(label='YES — A MEMBER RECRUITED ME',style=discord.ButtonStyle.primary,custom_id='recruit_referral_yes')
    async def yes(self,interaction:discord.Interaction,button:discord.ui.Button):
        await interaction.response.send_message('Select the **active 1/5 Cav member** who recruited you. Their recruiting credit will be tied to your Recruiting Case.',view=RecruiterMemberSelectView(),ephemeral=_recruit_ephemeral(interaction))

    @discord.ui.button(label='NO',style=discord.ButtonStyle.secondary,custom_id='recruit_referral_no')
    async def no(self,interaction:discord.Interaction,button:discord.ui.Button):
        await interaction.response.send_modal(RecruitFinalDetailsModal(recruited_by='NONE'))
...
class RecruitPart3View(discord.ui.View):
    ...
    async def part3(self,interaction:discord.Interaction,button:discord.ui.Button):
        await interaction.response.send_message('**WERE YOU RECRUITED BY AN ACTIVE 1/5 CAV MEMBER?**',view=RecruitReferralView(),ephemeral=_recruit_ephemeral(interaction))
...
async def _begin_or_resume_recruit_application(interaction:discord.Interaction):
    ...
    target_case=data.get('target_case') or {}
    ...
    await interaction.response.send_message(
        f"**1/5 CAV — RECRUITING OFFICE**{case_line}\nYour intake is {'ready to resume' if draft.get('answers') else 'ready to begin'}. "
        f"Complete Part **{step} of 3** below. Each completed section is saved automatically.\n\n"
...
class RecruitIntakePromptView(discord.ui.View):
    def __init__(self): super().__init__(timeout=None)
    @discord.ui.button(label='BEGIN / RESUME INTAKE',style=discord.ButtonStyle.success,custom_id='recruit_intake_prompt_begin')
...
def build_recruit_credentials_message(case: dict, provision: dict, *, site: str | None = None) -> str:
    """Build the acceptance/login DM for an approved Replacement.

    Acceptance is deliberately separate from placement. Do not imply that a
    Company, Platoon, Squad, MOS, or leader has been assigned when Command has
    only accepted the Recruiting Case.
    """
            ...
            try:
                if case:
                    ...
                    msg=("**WELCOME TO THE 1/5 CAV — REPLACEMENT LINE**\n\n"
                         ...
                         "**1 — LINK YOUR GAME ACCOUNT**\nRun **`/link-game`** and follow Battalion Clerk’s prompts.\n\n"
                         "**2 — REPORT IN**\nRun **`/apply`** to complete the short Discord intake. You can answer everything right here without leaving Discord.\n\n"
                         "**3 — STAND BY FOR ORDERS**\nOnce Command accepts your case, you’ll move to **Ready to Assign** until your outfit is filed.\n\n"
                         "If an active 1/5 Cav member recruited you, the intake will let you select that Soldier so the referral is attached to your case.\n\n"
                         "No mandatory operation schedule. No drawn-out processing. Get identified, get processed, and get in the fight.\n\n"
                         "**BATTALION CLERK • 1/5 CAV**")
                if msg:
                    await member.send(msg)
...
async def commend(interaction:discord.Interaction, member:discord.Member, category:app_commands.Choice[str], message:str):
    """File a peer commendation with one authoritative core INSERT.

    The command intentionally does not depend on optional identity columns or write
    service history in the same transaction. Once the core INSERT returns an id, the
    commendation is committed and cannot be rolled back by secondary bookkeeping.
    """
    if not interaction.guild:
    ...
    try:
        ...
        commend_db=getattr(collector,'db',None)
        if not commend_db or not getattr(commend_db,'pool',None):
            await _reply('Battalion Clerk cannot reach the personnel database right now. Nothing was filed.'); return
        ...
        async with commend_db.pool.acquire() as conn:
            ...
            # commendation schema from V33 forward.
            duplicate=await conn.fetchval("""SELECT 1 FROM personnel_commendations
                WHERE giver_personnel_id=$1 AND recipient_personnel_id=$2
                  AND category_code=$3 AND removed_at IS NULL
                  AND created_at > NOW()-INTERVAL '24 hours' LIMIT 1""",
                giver['id'],recipient['id'],category.value)
            if duplicate:
            ...
            daily=await conn.fetchval("""SELECT COUNT(*) FROM personnel_commendations
                WHERE giver_personnel_id=$1 AND removed_at IS NULL
                  AND created_at::date=CURRENT_DATE""",giver['id'])
            if int(daily or 0)>=5:
                await _reply('You have reached the 5-commendation daily limit. Save the next one for tomorrow.'); return

            # THIS is the filing action. A single autocommitted INSERT against only
            # the original V33 columns. No secondary statement can roll it back.
            filed=await asyncio.wait_for(conn.fetchrow("""INSERT INTO personnel_commendations(
                    recipient_personnel_id,giver_personnel_id,guild_id,category_code,message,source)
                VALUES($1,$2,$3,$4,$5,'DISCORD')
                RETURNING id,recipient_personnel_id,created_at""",
                recipient['id'],giver['id'],interaction.guild.id,category.value,reason),timeout=8.0)

        if not filed:
            raise RuntimeError('Database returned no commendation row after INSERT')

        # Optional identity enrichment happens AFTER the core row is committed.
        # It may fail on an older schema without affecting the filed commendation.
        try:
            await commend_db.execute("""UPDATE personnel_commendations
                SET recipient_discord_user_id=$1,giver_discord_user_id=$2
                WHERE id=$3""",member.id,interaction.user.id,filed['id'])
        except Exception as exc:
            log.warning('[COMMENDATION IDENTITY ENRICHMENT SKIPPED] id=%s error=%s',filed['id'],exc)
        ...
        log.exception('[COMMENDATION COMMAND TIMEOUT] guild=%s giver=%s recipient=%s',interaction.guild.id,interaction.user.id,member.id)
        await _reply('Battalion Clerk timed out while filing that commendation. **Nothing was confirmed as filed.** Try again once.')
    except Exception as exc:
...
async def link_game(interaction:discord.Interaction, platform:app_commands.Choice[str], game_id:str):
    ...
    if not result.get('ok') and 'No active Soldier Record is linked' in str(result.get('error') or ''):
        try:
            ...
            if staged.get('ok'):
                await interaction.followup.send(
                    f"**HLL: VIETNAM GAME IDENTITY — FILED FOR RECRUITING**\nPlatform: **{platform.name}**\nIdentity: `{identity}`\n\n"
                    "You are still on the Replacement line, so there is no Soldier Record to attach telemetry to yet. Battalion Clerk has filed this identity with your recruiting intake. When Command accepts your case, it will carry into your Soldier Record automatically.\n\n"
                    "Next: run **`/apply`** and finish the short Discord intake.",ephemeral=True); return
        except Exception as exc:
