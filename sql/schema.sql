    ...
    LOOP
        ...
        FOR p IN 1..3 LOOP
            ...
            VALUES(company_id, company_letter||'-P'||p||'-1-5',
                   CASE p WHEN 1 THEN '1st Platoon' WHEN 2 THEN '2nd Platoon' ELSE '3rd Platoon' END,
                   'Platoon', 10+p, TRUE)
            ON CONFLICT(unit_code) DO UPDATE SET parent_id=EXCLUDED.parent_id,display_name=EXCLUDED.display_name,unit_type=EXCLUDED.unit_type,sort_order=EXCLUDED.sort_order,is_active=TRUE;
            ...
            FOR s IN 1..3 LOOP
                ...
                VALUES(platoon_id, company_letter||'-P'||p||'-S'||s||'-1-5',
                       CASE s WHEN 1 THEN '1st Squad' WHEN 2 THEN '2nd Squad' ELSE '3rd Squad' END,
                       'Squad', 20+s, TRUE)
                ON CONFLICT(unit_code) DO UPDATE SET parent_id=EXCLUDED.parent_id,display_name=EXCLUDED.display_name,unit_type=EXCLUDED.unit_type,sort_order=EXCLUDED.sort_order,is_active=TRUE;
            END LOOP;

            -- Alpha/Bravo fire teams are persistent organizational children of each rifle squad.
            -- Personnel remain assigned to the squad node with fire_team as the final assignment layer.
            FOR s IN 1..3 LOOP
                INSERT INTO unit_nodes(parent_id,unit_code,display_name,unit_type,sort_order,is_active)
                SELECT sq.id, company_letter||'-P'||p||'-S'||s||'-A-1-5', 'Alpha Team', 'Team', 31, TRUE
                FROM unit_nodes sq WHERE sq.unit_code=company_letter||'-P'||p||'-S'||s||'-1-5'
                ON CONFLICT(unit_code) DO UPDATE SET parent_id=EXCLUDED.parent_id,display_name=EXCLUDED.display_name,unit_type=EXCLUDED.unit_type,sort_order=EXCLUDED.sort_order,is_active=TRUE;
                INSERT INTO unit_nodes(parent_id,unit_code,display_name,unit_type,sort_order,is_active)
                SELECT sq.id, company_letter||'-P'||p||'-S'||s||'-B-1-5', 'Bravo Team', 'Team', 32, TRUE
                FROM unit_nodes sq WHERE sq.unit_code=company_letter||'-P'||p||'-S'||s||'-1-5'
                ON CONFLICT(unit_code) DO UPDATE SET parent_id=EXCLUDED.parent_id,display_name=EXCLUDED.display_name,unit_type=EXCLUDED.unit_type,sort_order=EXCLUDED.sort_order,is_active=TRUE;
            END LOOP;
...
-- Canonical formation spelling. Historical rows and Discord aliases previously mixed
-- uppercase labels, "2d/3d", and title-case labels. Collapse them into one website value.
UPDATE unit_nodes SET display_name='1st Platoon' WHERE unit_type='Platoon' AND UPPER(display_name) IN ('1ST PLATOON','FIRST PLATOON');
UPDATE unit_nodes SET display_name='2nd Platoon' WHERE unit_type='Platoon' AND UPPER(display_name) IN ('2D PLATOON','2ND PLATOON','SECOND PLATOON');
UPDATE unit_nodes SET display_name='3rd Platoon' WHERE unit_type='Platoon' AND UPPER(display_name) IN ('3D PLATOON','3RD PLATOON','THIRD PLATOON');
UPDATE unit_nodes SET display_name='4th Platoon' WHERE unit_type='Platoon' AND UPPER(display_name) IN ('4TH PLATOON','FOURTH PLATOON');
UPDATE unit_nodes SET display_name='1st Squad' WHERE unit_type='Squad' AND UPPER(display_name) IN ('1ST SQUAD','FIRST SQUAD');
UPDATE unit_nodes SET display_name='2nd Squad' WHERE unit_type='Squad' AND UPPER(display_name) IN ('2D SQUAD','2ND SQUAD','SECOND SQUAD');
UPDATE unit_nodes SET display_name='3rd Squad' WHERE unit_type='Squad' AND UPPER(display_name) IN ('3D SQUAD','3RD SQUAD','THIRD SQUAD');
UPDATE unit_nodes SET display_name='4th Squad' WHERE unit_type='Squad' AND UPPER(display_name) IN ('4TH SQUAD','FOURTH SQUAD');

UPDATE personnel SET platoon=CASE UPPER(TRIM(platoon))
  WHEN '1ST PLATOON' THEN '1st Platoon' WHEN 'FIRST PLATOON' THEN '1st Platoon'
  WHEN '2D PLATOON' THEN '2nd Platoon' WHEN '2ND PLATOON' THEN '2nd Platoon' WHEN 'SECOND PLATOON' THEN '2nd Platoon'
  WHEN '3D PLATOON' THEN '3rd Platoon' WHEN '3RD PLATOON' THEN '3rd Platoon' WHEN 'THIRD PLATOON' THEN '3rd Platoon'
  WHEN '4TH PLATOON' THEN '4th Platoon' WHEN 'FOURTH PLATOON' THEN '4th Platoon' ELSE platoon END,
  squad=CASE UPPER(TRIM(squad))
  WHEN '1ST SQUAD' THEN '1st Squad' WHEN 'FIRST SQUAD' THEN '1st Squad'
  WHEN '2D SQUAD' THEN '2nd Squad' WHEN '2ND SQUAD' THEN '2nd Squad' WHEN 'SECOND SQUAD' THEN '2nd Squad'
  WHEN '3D SQUAD' THEN '3rd Squad' WHEN '3RD SQUAD' THEN '3rd Squad' WHEN 'THIRD SQUAD' THEN '3rd Squad'
  WHEN '4TH SQUAD' THEN '4th Squad' WHEN 'FOURTH SQUAD' THEN '4th Squad' ELSE squad END,
  fire_team=CASE UPPER(TRIM(fire_team)) WHEN 'ALPHA' THEN 'Alpha Team' WHEN 'ALPHA TEAM' THEN 'Alpha Team' WHEN 'BRAVO' THEN 'Bravo Team' WHEN 'BRAVO TEAM' THEN 'Bravo Team' ELSE fire_team END;

UPDATE assignment_history SET platoon=CASE UPPER(TRIM(platoon))
  WHEN '1ST PLATOON' THEN '1st Platoon' WHEN 'FIRST PLATOON' THEN '1st Platoon'
  WHEN '2D PLATOON' THEN '2nd Platoon' WHEN '2ND PLATOON' THEN '2nd Platoon' WHEN 'SECOND PLATOON' THEN '2nd Platoon'
  WHEN '3D PLATOON' THEN '3rd Platoon' WHEN '3RD PLATOON' THEN '3rd Platoon' WHEN 'THIRD PLATOON' THEN '3rd Platoon'
  ELSE platoon END,
...
-- Resolve legacy company-only node links down to the actual squad when the canonical
-- platoon/squad labels identify a unique active node.
UPDATE personnel p SET unit_node_id=sq.id
FROM unit_nodes sq JOIN unit_nodes pl ON pl.id=sq.parent_id JOIN unit_nodes co ON co.id=pl.parent_id
WHERE sq.unit_type='Squad' AND pl.unit_type='Platoon' AND co.unit_type='Company'
  AND p.separated_at IS NULL
  AND p.platoon=pl.display_name AND p.squad=sq.display_name
  AND ((p.unit_code ILIKE 'A/%' AND co.unit_code='A-1-5') OR (p.unit_code ILIKE 'B/%' AND co.unit_code='B-1-5') OR (p.unit_code ILIKE 'C/%' AND co.unit_code='C-1-5'));
...
ALTER TABLE recruiting_cases ADD COLUMN IF NOT EXISTS steam_id64 TEXT;
ALTER TABLE recruiting_cases ADD COLUMN IF NOT EXISTS game_platform TEXT;
ALTER TABLE recruiting_cases ADD COLUMN IF NOT EXISTS game_identity TEXT;
...
ALTER TABLE recruiting_cases ADD COLUMN IF NOT EXISTS credentials_resend_requested_at TIMESTAMPTZ;
ALTER TABLE recruiting_cases ADD COLUMN IF NOT EXISTS credentials_resend_requested_by TEXT;
ALTER TABLE recruiting_cases ADD COLUMN IF NOT EXISTS credentials_resend_rotate BOOLEAN NOT NULL DEFAULT FALSE;

CREATE TABLE IF NOT EXISTS recruit_credential_delivery_events (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    recruiting_case_id UUID NOT NULL REFERENCES recruiting_cases(id) ON DELETE CASCADE,
    event_type TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS recruit_credential_delivery_events_case_idx
    ON recruit_credential_delivery_events(recruiting_case_id,created_at DESC);
...
ALTER TABLE welcome_packets ADD COLUMN IF NOT EXISTS activated_at TIMESTAMPTZ;
ALTER TABLE welcome_packets ADD COLUMN IF NOT EXISTS submitted_at TIMESTAMPTZ;
ALTER TABLE welcome_packets ADD COLUMN IF NOT EXISTS approved_at TIMESTAMPTZ;
ALTER TABLE welcome_packets ADD COLUMN IF NOT EXISTS approved_by TEXT;
ALTER TABLE welcome_packets ADD COLUMN IF NOT EXISTS returned_at TIMESTAMPTZ;
ALTER TABLE welcome_packets ADD COLUMN IF NOT EXISTS returned_by TEXT;
ALTER TABLE welcome_packets ADD COLUMN IF NOT EXISTS return_note TEXT;
UPDATE welcome_packets SET approved_at=COALESCE(approved_at,completed_at),approved_by=COALESCE(approved_by,'LEGACY ONBOARDING') WHERE status='COMPLETE' AND completed_at IS NOT NULL;
...
-- 2026-08-24 — PERSONNEL AUTHORITY / AUTOMATION CONTROL LAYER
...
CREATE INDEX IF NOT EXISTS automation_ledger_person_idx ON automation_ledger(personnel_id,updated_at DESC);
CREATE INDEX IF NOT EXISTS automation_ledger_status_idx ON automation_ledger(status,updated_at DESC);
...
CREATE TABLE IF NOT EXISTS discord_managed_role_registry (
    guild_id BIGINT NOT NULL,
...
CREATE INDEX IF NOT EXISTS discord_managed_role_registry_key_idx ON discord_managed_role_registry(guild_id,canonical_key);

CREATE TABLE IF NOT EXISTS discord_sync_observations (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    personnel_id UUID NOT NULL REFERENCES personnel(id) ON DELETE CASCADE,
    guild_id BIGINT,
    discord_user_id BIGINT,
    ...
    actual_roles_json JSONB NOT NULL DEFAULT '[]'::jsonb,
    changes_json JSONB NOT NULL DEFAULT '{}'::jsonb,
    error_text TEXT,
    observed_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
CREATE INDEX IF NOT EXISTS discord_sync_observations_person_idx ON discord_sync_observations(personnel_id,observed_at DESC);
ALTER TABLE discord_role_sync_queue ADD COLUMN IF NOT EXISTS attempt_count INTEGER NOT NULL DEFAULT 0;
ALTER TABLE discord_role_sync_queue ADD COLUMN IF NOT EXISTS last_attempt_at TIMESTAMPTZ;
ALTER TABLE discord_role_sync_queue ADD COLUMN IF NOT EXISTS next_retry_at TIMESTAMPTZ;

-- Automatic game identity linkage filed from recruiting applications.
ALTER TABLE recruiting_cases ADD COLUMN IF NOT EXISTS game_identity_link_status TEXT;
ALTER TABLE recruiting_cases ADD COLUMN IF NOT EXISTS game_identity_link_error TEXT;
ALTER TABLE recruiting_cases ADD COLUMN IF NOT EXISTS game_identity_linked_at TIMESTAMPTZ;
...
CREATE INDEX IF NOT EXISTS hll_identity_claims_status_idx ON hll_identity_claims(status,platform,created_at);
CREATE INDEX IF NOT EXISTS hll_identity_claims_personnel_idx ON hll_identity_claims(personnel_id,status);
CREATE INDEX IF NOT EXISTS hll_identity_claims_identity_idx ON hll_identity_claims(platform,normalized_identity,status);


-- ---------------------------------------------------------------------------
-- DISCORD-FIRST RECRUITING INTAKE
-- Discord is a second front door into the same Recruiting Case system. Drafts
-- are resumable and are never visible as Command applications until submitted.
-- ---------------------------------------------------------------------------
ALTER TABLE recruiting_cases ADD COLUMN IF NOT EXISTS application_source TEXT NOT NULL DEFAULT 'WEBSITE';
ALTER TABLE recruiting_cases ADD COLUMN IF NOT EXISTS recruited_by_text TEXT;

CREATE TABLE IF NOT EXISTS discord_recruiting_intake (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    guild_id BIGINT NOT NULL,
    discord_user_id BIGINT NOT NULL,
    ...
    status TEXT NOT NULL DEFAULT 'IN_PROGRESS',
    submitted_case_id UUID REFERENCES recruiting_cases(id) ON DELETE SET NULL,
    started_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    submitted_at TIMESTAMPTZ,
    UNIQUE(guild_id, discord_user_id)
);
CREATE INDEX IF NOT EXISTS discord_recruiting_intake_status_idx
ON discord_recruiting_intake(status,updated_at DESC);

-- 2026-08-26 — Server-authoritative career / seeding telemetry support.
CREATE TABLE IF NOT EXISTS career_reconciliation_status (
    id INTEGER PRIMARY KEY DEFAULT 1,
    last_run_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    checked INTEGER NOT NULL DEFAULT 0,
    linked INTEGER NOT NULL DEFAULT 0,
    readiness_updates INTEGER NOT NULL DEFAULT 0,
    mos_updates INTEGER NOT NULL DEFAULT 0,
    ribbons_awarded INTEGER NOT NULL DEFAULT 0,
...
CREATE INDEX IF NOT EXISTS hll_seeding_service_person_idx ON hll_seeding_service(personnel_id,service_date DESC);


-- Discord delivery receipt for high-value Soldier notices (awards/promotions).
CREATE TABLE IF NOT EXISTS discord_notification_delivery (
    notification_id UUID PRIMARY KEY REFERENCES soldier_notifications(id) ON DELETE CASCADE,
    personnel_id UUID NOT NULL REFERENCES personnel(id) ON DELETE CASCADE,
    status TEXT NOT NULL DEFAULT 'PENDING',
    error_text TEXT,
...
CREATE INDEX IF NOT EXISTS discord_notification_delivery_person_idx ON discord_notification_delivery(personnel_id,status);
